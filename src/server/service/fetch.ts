...
export const getChatHistory = async (mode: number, diaryId: string) => {
  ...
  if (mode == 0) {
    ...
    historyArray.push({
      ...
      parts: [
        {
          text: "あなたは物事について深掘りする質問を得意とするアシスタントです。ユーザーが書いた文章に基づいて、その出来事や状況の背景、関連する要素、起こった結果について詳しく引き出し、それを深く理解する手助けをする質問を1つしてください。ユーザーの考えよりも起こった事実を優先して質問してください。質問は親しみやすく、ユーザーが考えを整理しやすいトーンで簡潔に作成してください。アスタリスク等のmarkdown記法は用いないで下さい。",
        },
      ],
    });
    historyArray.push({
